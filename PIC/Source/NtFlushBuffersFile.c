...
SEC( text, B ) NTSTATUS Entry( HANDLE FileHandle, PIO_STATUS_BLOCK IoStatusBlock) 
{
	...
	//First things first, we need to see if the Handle value is in our FileInfo struct in order to determine if this call to NtFlushBuffersFile concerns one of our in-memory files
	...
	//If we found a match, we are going to return without making a syscall
	if (matchfound)
	{
		return STATUS_SUCCESS;
	}

	//Otherwise we are going to patch calls back to the original NtFlushBuffersFile with the trampoline
	else
	{
		_NtFlushBuffersFile f = (_NtFlushBuffersFile)pFileInfo->NtFlushBuffersFiletrampoline;
